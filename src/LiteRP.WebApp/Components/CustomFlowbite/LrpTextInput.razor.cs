...
/// </summary>
public partial class LrpTextInput<TValue> : IDisposable
{
    ...
    private string GetWrapperClasses() => CombineClasses(BaseWrapperClasses);
    ...
    private string GetInputClasses()
    {
        ...
        // Add size classes
        var (leftClasses, rightClasses, sizeClasses) = Size switch
        {
            TextInputSize.Small => ("pl-2", "pr-2", "py-2 text-sm"),
            TextInputSize.Large => ("pl-4", "pr-4" , "sm:text-md py-4"),
            _ => ("pl-2.5", "pr-2.5", "p-2.5 text-sm") // Medium (default)
        };

        classes.Add(sizeClasses);

        // Add icon padding
        classes.Add(Icon != null ? "pl-10" : leftClasses);
        classes.Add(RightIcon != null ? "pr-10" : rightClasses);
